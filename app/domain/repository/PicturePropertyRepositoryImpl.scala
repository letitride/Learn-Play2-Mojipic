...
class PicturePropertyRepositoryImpl extends PicturePropertyRepository {
      ...
      using(DB(ConnectionPool.borrow())){ db =>
        db.readOnly{ implicit session =>
          ...
          sql.map( resultSetToPictureProperty).single().apply()
            .getOrElse(throw new RuntimeException(s"Picture is not found. PictureId: ${pictureId.value}"))
  ...
  override def findAllByTwitterIdAndDateTime(twitterId: TwitterId, toDateTime: LocalDateTime): Future[Seq[PictureProperty]] = {
    Future.fromTry(Try{
      using(DB(ConnectionPool.borrow())){db =>
        db.readOnly{ implicit session =>
          val sql =
  ...
  override def findAllByDateTime(toDateTime: LocalDateTime): Future[Seq[PictureProperty]] = {
    Future.fromTry(Try{
      using(DB(ConnectionPool.borrow())){db =>
        db.readOnly{ implicit session =>
          val sql =
